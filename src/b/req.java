...
//		assertaccess();
		...
		if(contentType!=null){
			if(contentType.startsWith("dir;")){
				...
				reply("204 No Content".getBytes(),null,null,null);
				state=state_nextreq;
				return;
			}
...
//	}
	...
	private void reply(final byte[]resultcode,final byte[]lastMod,final byte[]contentType,final byte[]content)throws Throwable{
		final ByteBuffer[]bb=new ByteBuffer[16];
		int bi=0;
...
//			System.out.println(path_s+"   content upload done");
			reply("204 No Content".getBytes(),null,null,null);
			state=state_nextreq;
		}
	}
