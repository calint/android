...
final class oschunked extends OutputStream{
	...
	private byte[]buf;
	private int bufi;
	oschunked(final req r,final int chunk_size_bytes)throws Throwable{this.r=r;this.chunk_size_bytes=chunk_size_bytes;chunkhx=(Integer.toHexString(chunk_size_bytes)+"\r\n").getBytes();buf=new byte[chunk_size_bytes];}
	...
	private void write_blocking(final ByteBuffer[]bba)throws IOException{
		...
		while(remaining!=0){
			...
			if(c==0)synchronized(r){
				...
				r.selkey.selector().wakeup();//? seletor may notify before next line
	...
	void finish()throws InterruptedException,IOException{flush();write_blocking(new ByteBuffer[]{bb_eochunk.slice()});}
}
