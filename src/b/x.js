...
//		this._t0=new Date().getMilliseconds();
		$d(new Date().getMilliseconds()-this._t0+" * sending");
		...
		break;
	case 2:
		$d(new Date().getMilliseconds()-this._t0+" * sending done");
		break;
	case 3:
		$d(new Date().getMilliseconds()-this._t0+" * reply code "+this.status);
		var s=this.responseText.charAt(this.responseText.length-1);
		if(s!='\n'){
			$d(new Date().getMilliseconds()-this._t0+" * not eol "+(this.responseText.length-this._jscodeoffset));
			break;
		}
		var jscode=this.responseText.substring(this._jscodeoffset);
		$d(new Date().getMilliseconds()-this._t0+" * run "+jscode.length+" bytes");
		$d(jscode);
...
//		$d(" * done");
		...
		this._dt=new Date().getMilliseconds()-this._t0;//? var _dt
		$d("~~~~~~~ ~~~~~~~ ~~~~~~~ ~~~~~~~ ")
...
//		$d(" * has connection");
	...
	ui.req._t0=new Date().getMilliseconds();
	ui.req._pd=post;
