...
final public class activity extends Activity implements Runnable,SurfaceHolder.Callback{
	public static String cluketName="c.a.t.tapit";
	public static interface cluket extends Serializable{
		void paint(final state state,final Canvas canvas);
		void update(final state state);
	}
	final public static class state implements Serializable{
	...
	private void thread_start(){
		thread=new Thread(this,activity.class.getName());
		on=true;
		thread.start();
	}
	...
	public void run(){
		...
		while(on){
			...
			fps_c++;
			state.frameno++;
			state.cluket.paint(state,canvas);
			float x=0f,y=12f;
			for(byte b:state.keys){
				if((b&1)!=0){
			...
			state.cluket.update(state);
			long t1=System.currentTimeMillis();
			long fps_dt=t1-fps_t0;
			if(fps_dt>1000){
