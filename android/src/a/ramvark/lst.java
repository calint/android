...
public abstract class lst extends a implements $.labeled{
	...
	final public void to(final xwriter x)throws Throwable{
		...
		frstid=null;
		rendhead(x);
		foreach(qry.toString(),new cstore.visitable(){public void visit(final itm e){
			if(frstnm==null){
				rendlisthead(x);
	...
	public void foreach(final String q,final cstore.visitable cv)throws Throwable{
		if(owner==null){
...
//		final Scanner sc=new Scanner(toString());
	...
	protected void rendhead(final xwriter x){
		...
		x.ax(this,"cr","⌾").spc();
		x.nl().style();
		x.css("input.q","width:20em;background:#fff;padding:.5em 2em .5em 1em;border:0px dotted #020;box-shadow:0 0 .5em rgba(0,0,0,.5);border-radius:0em");			
		x.styleEnd();
		x.nl().inputax(qry,"q",this,"ch","sl");
		x.focus(qry);
	}
